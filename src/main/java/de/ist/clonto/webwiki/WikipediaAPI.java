...
public class WikipediaAPI {
	...
	public static boolean isValidCategory(String title) {
		...
		List<Page> pages = connector.queryInfo(user, Arrays.asList(new String[] { title }));
		if (!pages.isEmpty()) {
	...
	public static List<String> getPages(String category) throws SAXException, IOException, InterruptedException {
	...
	public static List<String> getCategoryMembers(String category, Cmtype cmtype)
		...
		// , "clshow", "!hidden"
		do {
			String[] query = new String[] { "list", "categorymembers", "cmtitle", "Category:" + category, "cmtype",
					cmtype.id, "cmlimit", "max", "cmcontinue", cmcontinue };

			String rawXmlResponse = processQuery(query);

			XMLCategoryMembersParser parser = new XMLCategoryMembersParser(rawXmlResponse);

			parser.parse();

			List<PageInfo> pages = parser.getPagesList();

			// BUGFIX Imagine the title ?: as in
	...
	private static String processQuery(String[] query) throws InterruptedException {
		int i = 0;
	...
	public static void main(String[] args) {
		try {
			getPages("Computer languages").forEach(c -> System.out.println("c:" + c));
